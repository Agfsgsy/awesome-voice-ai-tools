...
class _VoiceCloneScreenState extends ConsumerState<VoiceCloneScreen> {
  ...
  Future<void> _submit() async {
    ...
    if (!reference.analysis.clearSpeech) {
      showArabicError(context, 'التسجيل لا يحتوي كلامًا واضحًا؛ أعد التسجيل قبل الاستنساخ.');
      return;
    }
    if (!_consent || _statementController.text.trim().length < 12) {
      showArabicError(context, 'يجب تأكيد ملكية الصوت أو وجود إذن صريح وكتابة إقرار واضح.');
      return;
    }
    ...
    try {
      final job = await ref.read(apiServiceProvider).cloneVoice(
        ...
        SectionCard(
          ...
          child: reference == null
              ? Column(
                  ...
                  children: <Widget>[
                    const Text('اختر تسجيلًا واضحًا مدته ثانيتان على الأقل. سيُحلل قبل السماح بالاستنساخ.'),
                    const SizedBox(height: 12),
                    Wrap(
                      spacing: 10,
                      runSpacing: 10,
                      children: <Widget>[
                        FilledButton.tonalIcon(onPressed: _busy ? null : () => context.go('/record'), icon: const Icon(Icons.mic_rounded), label: const Text('تسجيل وتحليل')),
                        OutlinedButton.icon(onPressed: _busy ? null : _chooseReference, icon: const Icon(Icons.folder_open_rounded), label: const Text('اختيار ملف وتحليله')),
                      ],
                    ),
                  ],
                )
              : ListTile(
                  ...
                  subtitle: Text('جودة ${reference.analysis.qualityScore}/100 • ${reference.analysis.durationSeconds.toStringAsFixed(1)} ثانية'),
                  trailing: IconButton(tooltip: 'تغيير الملف', onPressed: _chooseReference, icon: const Icon(Icons.swap_horiz_rounded)),
                ),
        ),
        if (reference != null) ...<Widget>[const SizedBox(height: 12), AudioAnalysisCard(analysis: reference.analysis)],
        const SizedBox(height: 12),
        SectionCard(
          title: 'الإقرار والنص',
            ...
            children: <Widget>[
              DropdownButtonFormField<String>(
                ...
                items: const <DropdownMenuItem<String>>[
                  DropdownMenuItem(value: 'self', child: Text('أنا صاحب الصوت')),
                  DropdownMenuItem(value: 'explicit_authorization', child: Text('لدي إذن صريح من صاحب الصوت')),
                ],
                onChanged: (value) {
                  setState(() {
                    _rights = value ?? 'self';
              ...
              TextField(
                ...
                decoration: InputDecoration(
                  ...
                  hintText: _rights == 'self' ? 'اكتب: أنا صاحب هذا الصوت وأوافق على استنساخه.' : 'اكتب وصف الإذن الصريح الذي حصلت عليه.',
                ),
              ...
              CheckboxListTile(
                ...
                title: Text(_rights == 'self' ? 'أؤكد أنني صاحب الصوت وأوافق على استنساخه.' : 'أؤكد أن لدي إذنًا صريحًا وقابلًا للإثبات من صاحب الصوت.'),
                onChanged: _statementController.text.trim().length < 12 ? null : (value) => setState(() => _consent = value ?? false),
              ),
              const Divider(height: 28),
              TextField(controller: _textController, minLines: 4, maxLines: 10, decoration: const InputDecoration(labelText: 'النص المراد استنساخه')),
              const SizedBox(height: 12),
              DropdownButtonFormField<String>(
              ...
              Slider(value: _candidates.toDouble(), min: 2, max: 5, divisions: 3, onChanged: (value) => setState(() => _candidates = value.round())),
              FilledButton.icon(
                onPressed: _busy || !_consent ? null : _submit,
                icon: _busy ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.record_voice_over_rounded),
