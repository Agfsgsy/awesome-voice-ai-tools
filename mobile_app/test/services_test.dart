...
void main() {
  ...
  test('يفك رمز اقتران QR الصحيح ويرفض الرمز الناقص', () {
    ...
    expect(() => PairingPayload.parse('voiceai://pair?id=short'), throwsA(isA<AppException>()));
  });

  test('يسمح HTTP داخل LAN ويلزم HTTPS خارجيًا', () {
    expect(ApiService.normalizeServerUrl('http://192.168.1.8:8000/'), 'http://192.168.1.8:8000');
    expect(ApiService.normalizeServerUrl('https://voice.example.com/api'), 'https://voice.example.com');
    expect(() => ApiService.normalizeServerUrl('http://voice.example.com'), throwsA(isA<AppException>()));
  });
