...
snapshot of the enabled key pool and retry each key before falling back.
"""
...
def _detail(response: httpx.Response) -> str:
    try:
        ...
        if isinstance(data, dict):
            return str((data.get("error") or {}).get("message") or data.get("message") or response.text[:600])
    except Exception:
        pass
...
def stable_record_result(key: str, status: str, detail: str = "", retry_after: float | None = None) -> None:
    ...
    wait = int(max(15, min(300, float(retry_after or (45 if status in {"quota", "rate_limited"} else 20)))))
    friendly = (
    ...
    record.update(
        ...
        last_transient_detail=detail[:600],
...
async def stable_scene_audio(
    ...
    """Retry and rotate one key at a time without changing the saved key list mid-job."""
                ...
                for attempt in (1, 2, 3):
                    ...
                    try:
                        ...
                        if response.status_code == 429:
                            ...
                            errors.append(f"المفتاح {key_index}: حد مؤقت 429")
                            abandon_key = True
                            break
                        ...
                        if response.status_code == 403:
                            ...
                            errors.append(f"المفتاح {key_index}: مرفوض")
                            abandon_key = True
                            break
                        if response.status_code in {400, 404}:
                            errors.append(f"{model}: غير متاح")
                            break
                        if response.status_code >= 500:
                            if attempt < 3:
                                await asyncio.sleep(_delay(response, attempt))
                                continue
                            stable_record_result(key, "network", _detail(response), 20)
                        ...
                        if attempt < 3:
                            ...
                            continue
                        stable_record_result(key, "network", f"{type(exc).__name__}: {exc}", 20)
                        errors.append(f"المفتاح {key_index}: اتصال مؤقت")
                if abandon_key:
                    break
    raise HTTPException(status_code=502, detail="تعذر مشهد Gemini مؤقتًا من دون فصل المفاتيح. " + "; ".join(errors[-6:]))
