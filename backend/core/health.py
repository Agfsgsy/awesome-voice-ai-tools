...
def check_disk() -> Dict:
    try:
        ...
        return {
            ...
            "detail": f"{
                free_gb:.1f} GB free",
            "message": "OK" if ok else "Low disk"}
...
def check_models_dir() -> Dict:
    ...
    model_files = [f for f in models if f.is_file()
...
def check_plugins() -> Dict:
    try:
        ...
        plugins = [f for f in PLUGINS_DIR.iterdir() if f.suffix ==
                   ".py" and not f.name.startswith("_")]
        return {
            "name": "Plugins",
            "ok": True,
            "detail": f"{
...
def run_all_checks(port: int = 8000) -> List[Dict]:
    ...
    for check_fn in [
        ...
        try:
            ...
            checks.append({"name": check_fn.__name__, "ok": False,
                          "detail": str(e), "message": "Check failed"})
    return checks
