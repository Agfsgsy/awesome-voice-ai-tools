...
# استوديو ابن الواقدي 6.0 — Ultimate Voice

استوديو ثنائي اللغة لإنتاج الصوت والمقابلات وكتابة النصوص. يبدأ بمحركات مجانية، ويفصل بوضوح بين المجاني والتجريبي والمدفوع، ولا يبدّل اللغة أو جنس المتحدث أو مزود الصوت بصمت.

## ما الذي يقدمه الإصدار 6.0؟

- واجهة عربية/إنجليزية كاملة مع بقاء اللغة المختارة محفوظة.
- كتالوج صارم يعرض الأصوات المطابقة للغة والجنس واللهجة فقط.
- صوت عربي وإنجليزي عصبي عبر **Microsoft Edge TTS** من دون مفتاح API.
- صوت عربي محلي عبر **Piper**؛ يُنزّل النموذج العربي تلقائيًا عند أول استخدام ثم يعمل دون اتصال.
- إنتاج صوت منفرد وحوار متعدد المتحدثين مع خريطة موثقة لكل دور وصوته.
- كاتب نصوص للتأليف وإعادة الصياغة والمقابلات عبر قالب محلي مجاني أو Gemini أو OpenAI.
- ربط مباشر واختبار حقيقي لـGemini وElevenLabs وAzure وGoogle Cloud وOpenAI.
- محركات مدفوعة وتجريبية لا تُستعمل إلا عندما يختارها المستخدم ويربط مفتاحها.
...
|---|---:|---:|---|
| `edge` | لا | مطلوب | مجاني، عربي وإنجليزي، وتصفية صارمة للجنس واللهجة |
| `piper` | لا | أول مرة فقط | محلي بعد تنزيل النموذج |
| `gemini` | نعم | مطلوب | أصوات توليدية وحصة بحسب حساب Google |
| `elevenlabs` | نعم | مطلوب | خطة مجانية محدودة وخطط مدفوعة؛ تُضبط معرّفات الأصوات لكل لغة وجنس |
| `azure` | نعم | مطلوب | طبقة F0 حيث تتوفر ثم مدفوع |
| `google_cloud` | نعم | مطلوب | Chirp 3 HD مع بيانات جنس رسمية |
| `openai` | نعم | مطلوب | مدفوع؛ أصواته تُعرض كمحايدة لعدم وجود تصنيف جنس رسمي ثابت |
...
يتحقق مدقق الإصدار من رقم النسخة وسياسة المجاني أولًا، وأجسام JSON، وعدم تكرار المسارات، وعقود الواجهة الثنائية والاستوديو الصارم.

## الملفات والخصوصية

- في النسخة المجمعة تُحفظ النماذج والمخرجات والسجلات داخل `%LOCALAPPDATA%\VoiceAIStudioArabic`.
- لا تُحفظ مفاتيح المزودين في المتصفح، ولا تُرسل إلى المستودع.
